class MenuButton4 {
  ...
  MenuButton4(float eSize_, float ePadding_) {
    ...
    eSize = eSize_;
    ePadding = ePadding_;
    buttonX = menu.x+eSize/2+ePadding;              //POSITION BUTTON 4 ON SCREEN
    buttonY = rbutton.buttonY + bbutton.eSize + button3.eSize  + rbutton.eSize + (ePadding-2)*3; //*3 is number of buttons above button
    isPressed = false;
  }
  ...
  void display() {
    ...
    if (rbutton.isPressed == false && bbutton.isPressed == false && button3.isPressed == false) {            /// if button one+two are FALSE, fill button 4
      fill(255);
